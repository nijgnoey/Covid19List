...
try {
	...
	gubun = jsonObject.get("gubun").toString();
	...
	// 쿼리
	if (gubun.equals("전체")) {
		String sql = "SELECT * FROM covid19Info WHERE stdDay = ?";
		...
		String sql = "SELECT * FROM covid19Info WHERE stdDay = ? AND gubun = ?";
		PreparedStatement stmt = con.prepareStatement(sql);
		stmt.setString(1, stdDay);
		stmt.setString(2, gubun);
		ResultSet rs = stmt.executeQuery();
		JSONArray jArray = new JSONArray();
		// 쿼리 실행
		while(rs.next()) {
